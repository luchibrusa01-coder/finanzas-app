...
function ActivoForm({ initial, onSave, onCancel }: {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    onSave({ nombre: nombre.trim(), monto: m, tipo })
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-gray-50 dark:bg-gray-700/40 rounded-xl p-3 space-y-2 mt-2">
      <input
        ...
        placeholder="Nombre (ej: SPY ETF, Bitcoin...)"
        value={nombre}
  ...
  return (
    <section className="card">
      <div className="flex justify-between items-center mb-3">
        <h2 className="section-title mb-0">Activos en USD</h2>
        {!showAdd && (
              ...
              {editingId === a.id ? (
                ...
                <div className="flex justify-between items-center py-2 gap-2">
                  <div className="min-w-0">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${TIPO_COLORS[a.tipo]}`}>
                      {a.tipo}
                    </span>
                  </div>
      ...
      {activos.length > 0 && (
        <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700 flex justify-between font-bold">
          <span className="text-gray-700 dark:text-gray-300">Patrimonio total</span>
